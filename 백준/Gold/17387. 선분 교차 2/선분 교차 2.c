...
int onSegments(pair a, pair b, pair c) {
...
int Intersect(pair L1_first, pair L1_second, pair L2_first, pair L2_second) {
    ...
    if(!d1 && onSegments(L2_first, L2_second, L1_first)) return 1;
    if(!d2 && onSegments(L2_first, L2_second, L1_second)) return 1;
    if(!d3 && onSegments(L1_first, L1_second, L2_first)) return 1;
    if(!d4 && onSegments(L1_first, L1_second, L2_second)) return 1;
